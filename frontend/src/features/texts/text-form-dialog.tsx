    ...
    >
      <form id="text-form" className="space-y-3" onSubmit={submitForm} noValidate>
        ...
        <FormSection title="Metadata" description="Set core text identity before validating transcript and clips.">
          ...
          <FormField label="Repetitions" htmlFor="text-reps" hint="How many times to repeat each clip during practice (1-10).">
            <input
              id="text-reps"
              type="number"
              min={1}
              max={10}
              value={reps}
              onChange={(event) => setReps(Math.max(1, Math.min(10, parseInt(event.target.value, 10) || 1)))}
              className="h-10 rounded-md border border-slate-300 bg-white px-3 text-sm"
              required
            />
          </FormField>
