...
HELP_COMMANDS = [
    ...
    {
        "name": "keep",
        "description": "Accept your latest attempt and continue",
        "shortcut": "Alt+K",
    },
    {
        "name": "showdiff",
...
def _get_scoring_rule(db, key: str, *, fallback: float) -> float:
    row = db.execute(
        "SELECT points FROM scoring_rules WHERE key = ?",
        (key,),
    ).fetchone()
    if row is None:
        return fallback
...
def _get_owned_chapter_for_session(chapter_id: int):
    db = get_db()
    return db.execute(
        """
        SELECT
...
def _load_chapter_lines(chapter_id: int) -> dict[int, dict]:
    db = get_db()
    rows = db.execute(
        """
...
def _load_attempts(session_id: int) -> list[dict]:
    ...
    rows = db.execute(
        ...
        SELECT
          ...
          sa.is_correct,
          sa.created_at,
          cl.line_text AS expected_line_text
        FROM session_attempts sa
        LEFT JOIN chapter_lines cl ON cl.id = sa.chapter_line_id
        WHERE sa.session_id = ?
        ORDER BY sa.id ASC
        """,
        (session_id,),
    ...
    return [
        {
            "id": row["id"],
            "chapterLineId": row["chapter_line_id"],
...
def _story_text_from_chapter(chapter_id: int) -> str:
    db = get_db()
    rows = db.execute(
        """
        SELECT line_text
...
def _openai_tutor_response(*, story_text: str, line_text: str, attempt_text: str) -> tuple[str, str]:
    ...
    response = client.responses.create(
        ...
        input=[
            ...
            {
                "role": "user",
                "content": f"This is the line which I got wrong: {line_text}",
            },
            {
                "role": "user",
                "content": f"My attempt to write the line from memory was: {attempt_text}",
            },
        ],
    )
    ...
    if word_count_diff == 0:
        ...
        for index, word in enumerate(attempt_words):
            words.append(
                {
                    "word": word,
                    "isMatch": attempt_uncap[index] == expected_uncap[index],
                }
            )
        return {
...
def _session_payload(session_row, *, include_attempts: bool = True) -> dict:
    ...
    if clip_index is not None:
        lines_by_index = _load_chapter_lines(int(session_row["chapter_id"]))
        ...
        clip_url = f"/api/chapters/{session_row['chapter_id']}/clips/{clip_index}"

    return {
        "id": session_row["id"],
        "storyId": session_row["story_id"],
        "storyName": session_row["story_name"],
        "storyLevel": session_row["story_level"],
        "chapterId": session_row["chapter_id"],
        "chapterTitle": session_row["chapter_title"],
        "reps": reps,
...
@require_auth
def list_sessions():
    story_id = _parse_positive_int(request.args.get("story"))
    chapter_id = _parse_positive_int(request.args.get("chapter"))
    ...
    if request.args.get("story") and story_id is None:
        return error_response("VALIDATION_ERROR", "story must be a positive integer", 400)
    if request.args.get("chapter") and chapter_id is None:
        return error_response("VALIDATION_ERROR", "chapter must be a positive integer", 400)
    ...
    if story_id is not None:
        where_parts.append("c.story_id = ?")
        values.append(story_id)
    if chapter_id is not None:
        where_parts.append("se.chapter_id = ?")
    ...
    if date:
        where_parts.append("se.started_at >= ?")
        where_parts.append("se.started_at < ?")
        next_day = (datetime.strptime(date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")
        values.append(f"{date}T00:00:00Z")
        values.append(f"{next_day}T00:00:00Z")
    else:
        if date_from:
            where_parts.append("se.started_at >= ?")
            values.append(f"{date_from}T00:00:00Z")
        if date_to:
            next_day = (datetime.strptime(date_to, "%Y-%m-%d") + timedelta(days=1)).strftime(
                "%Y-%m-%d"
            )
            where_parts.append("se.started_at < ?")
            values.append(f"{next_day}T00:00:00Z")
...
@require_auth
def start_session(chapter_id: int):
    chapter = _get_owned_chapter_for_session(chapter_id)
    if chapter is None:
        return error_response("NOT_FOUND", "Chapter not found", 404)
    if not bool(chapter["is_ready"]):
        return error_response(
            "VALIDATION_ERROR",
            "Chapter must be validated and ready before starting a session",
            400,
        )
    ...
    total_clips = int(chapter["line_count"])
    if total_clips <= 0:
        return error_response(
            "VALIDATION_ERROR",
            "Chapter transcript has no lines to practice",
            400,
        )
    ...
    cursor = db.execute(
        ...
        (
            ...
            reps,
            total_clips,
            json.dumps({"cursor": 0}, separators=(",", ":")),
        ),
    )
    db.commit()
...
def create_attempt(session_id: int):
    ...
    if session_row["status"] != "in_progress":
        return error_response(
            "CONFLICT",
            "Cannot record attempts for a finished session",
            409,
        )
    ...
    if clip_index is None or rep_index is None:
        return error_response(
            "CONFLICT",
            "Session has no remaining clips; complete it",
            409,
        )

    if "clipIndex" in payload:
        requested_clip_index = _parse_positive_int(payload.get("clipIndex"))
        if requested_clip_index is None or requested_clip_index != clip_index:
            return error_response("CONFLICT", "clipIndex does not match session state", 409)
    if "repIndex" in payload:
        requested_rep_index = _parse_positive_int(payload.get("repIndex"))
        if requested_rep_index is None or requested_rep_index != rep_index:
            return error_response("CONFLICT", "repIndex does not match session state", 409)

    lines_by_index = _load_chapter_lines(int(session_row["chapter_id"]))
    expected_line = lines_by_index.get(clip_index)
    if expected_line is None:
        return error_response("INTERNAL_ERROR", "Missing chapter line for clip index", 500)
...
def create_session_event(session_id: int):
    ...
    if rep_index is not None:
        latest_attempt = _latest_attempt_for_position(
            session_id,
...
def create_tutor_feedback(session_id: int):
    ...
    if latest_attempt is not None and bool(latest_attempt["is_correct"]):
        return error_response(
            "VALIDATION_ERROR",
            "Tutor feedback is only available for incorrect attempts",
            400,
        )
    ...
    story_text = _story_text_from_chapter(int(session_row["chapter_id"]))
    if not story_text:
        return error_response("INTERNAL_ERROR", "Could not build story text for tutor", 500)
    ...
    try:
        ...
        logger.exception(
            "tutor_feedback_failed session_id=%s clip_index=%s",
            session_id,
            clip_index,
        )
        return error_response(
            "TUTOR_UNAVAILABLE",
            f"Tutor feedback is temporarily unavailable: {error}",
            503,
        )
