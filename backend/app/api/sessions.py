...
def _load_tutor_feedback_history(session_id: int, *, limit: int = 10) -> list[dict]:
    ...
    rows = db.execute(
        """
        SELECT id, clip_index, attempt_text, line_text, model_name, respose_json, created_at
        FROM tutor_feedback
        WHERE session_id = ?
        ORDER BY id DESC
        LIMIT ?
    ...
    result = []
    for row in rows:
        # Parse the JSON respose_json
        respose_json = row["respose_json"]
        try:
            corrections = json.loads(respose_json) if respose_json else {"corrections": []}
...
def create_tutor_feedback(session_id: int):
    ...
    respose_json = corrections.model_dump_json()
    ...
    insert_cursor = db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (session_id, clip_index, attempt_text, expected_line["text"], model_name, respose_json),
    )
    db.commit()
